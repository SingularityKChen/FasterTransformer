...
def build_dataset(is_train, config):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        # Get image filenames
        image_paths = [x[0] for x in dataset.imgs]
        image_names = [os.path.basename(path) for path in image_paths]
        # Replace dataset with tuples (data, target, image_name)
        dataset = [(data, target, image_name) for ((data, target), image_name) in zip(dataset, image_names)]
