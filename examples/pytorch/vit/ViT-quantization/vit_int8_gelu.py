...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        # Exporting related data of GELU.
        self.gelu_in = x.clone() # in
        x = self.act_fn(x)
        self.gelu_out = x.clone() # out
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.dropout(x)
        return x
    
    def get_gelu_data(self):
        # Copying a tensor from GPU memory to CPU memory and converting it to a NumPy array.
        gelu_in_cpu = self.gelu_in.cpu()
        gelu_out_cpu = self.gelu_out.cpu()
        gelu_in_np = gelu_in_cpu.numpy()
        gelu_out_np = gelu_out_cpu.numpy()

        return gelu_in_np, gelu_out_np
    
    def save_data(self, layer_index, gelu_in_np, gelu_out_np, image_names, output_dir):
        # If output_dir does not exist, create it
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Loop through batch_size and num_heads
        batch_size, width_len, height_len = gelu_in_np.shape
        for i in range(batch_size):
            # Generate unique filenames for each sub-tensor
            image_name = image_names[i] if image_names else "unnamed"
            input_filename = f"{image_name}_gelu_input_layer{layer_index}_batch_{i}.csv"
            output_filename = f"{image_name}_gelu_output_layer{layer_index}_batch_{i}.csv"
            ...
            input_df = pd.DataFrame(gelu_in_np[i])
            input_df.to_csv(input_filepath, index=False)

            output_df = pd.DataFrame(gelu_out_np[i])
...
class Block(nn.Module):
    ...
    def forward(self, x, image_names=None):
        ...
        # Obtain gelu input and output data.
        gelu_in_np, gelu_out_np = self.ffn.get_gelu_data()
        # Save gelu input and output data.
        self.ffn.save_data(self.layer_index, gelu_in_np, gelu_out_np, image_names, "./out_gelu")
