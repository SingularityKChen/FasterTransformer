...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # 存储未经 dropout 处理的 attention_probs
        self.raw_attention_probs = attention_probs.clone()
    ...
    def get_softmax_data(self):
        # 将张量从 GPU 内存复制到 CPU 内存，并转换为 NumPy 数组
        attention_scores_cpu = self.attention_scores.cpu()
        attention_probs_cpu = self.raw_attention_probs.cpu()
        attention_scores_np = attention_scores_cpu.numpy()
        attention_probs_np = attention_probs_cpu.numpy()

        return attention_scores_np, attention_probs_np

    def save_data(self, layer_index, attention_scores_np, attention_probs_np, image_names, output_dir):
        # If output_dir does not exist, create it
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Loop through attention_scores_np and attention_probs_np's batch_size and num_heads
        batch_size, num_heads, width_len, height_len = attention_scores_np.shape
        for i in range(batch_size):
            for j in range(num_heads):
                # Generate unique filenames for each sub-tensor
                image_name = image_names[i] if image_names else "unnamed"
                input_filename = f"{image_name}_softmax_input_layer{layer_index}_batch_{i}_head_{j}.csv"
                output_filename = f"{image_name}_softmax_output_layer{layer_index}_batch_{i}_head_{j}.csv"

                # Concatenate filename with output_dir to form full file paths
                input_filepath = os.path.join(output_dir, input_filename)
                output_filepath = os.path.join(output_dir, output_filename)
                
                '''
                # Save NumPy subarrays as CSV files
                with open(input_filepath, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerows(attention_scores_np[i, j])

                with open(output_filepath, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerows(attention_probs_np[i, j])
                ''' 
                # Save NumPy subarrays as CSV files using pandas
                input_df = pd.DataFrame(attention_scores_np[i, j])
                input_df.to_csv(input_filepath, index=False)

                output_df = pd.DataFrame(attention_probs_np[i, j])
...
class Block(nn.Module):
    def __init__(self, config, vis, layer_index):
    ...
    def forward(self, x, image_names=None):
        ...
        # 获取 softmax 输入和输出数据
        attention_scores_np, attention_probs_np = self.attn.get_softmax_data()
        # 保存 softmax 输入和输出数据
        self.attn.save_data(self.layer_index, attention_scores_np, attention_probs_np, image_names, "./out_softmax")
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for layer_index in range(config.transformer["num_layers"]):
            ...
            layer = Block(config, vis, layer_index)
            self.layer.append(copy.deepcopy(layer))
            #输出softmax函数的layer_idx
            print("layer_idx(num_layers):\t", layer_index)

    def forward(self, hidden_states, image_names=None):
        attn_weights = []
        for idx, layer_block in enumerate(self.layer):
            hidden_states, weights = layer_block(hidden_states, image_names)
            if self.vis:
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, image_names=None):
        embedding_output = self.embeddings(input_ids)
        encoded, attn_weights = self.encoder(embedding_output, image_names)
        return encoded, attn_weights
...
class VisionTransformerINT8(nn.Module):
    ...
    def forward(self, x, image_names=None, labels=None):
