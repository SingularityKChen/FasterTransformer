...
class Block(nn.Module):
    ...
    def forward(self, x, image_names=None):
        ...
        h = x
        if QUANT:
            # Exporting related data of Layernorm.
            self.layernorm_in = self.layernorm_input2_quantizer(x).clone() # in
            x = self.ffn_norm(self.layernorm_input2_quantizer(x))
            self.layernorm_out = x.clone() # out
        else:
            # Exporting related data of Layernorm.
            self.layernorm_in = x.clone() # in
            x = self.ffn_norm(x)
            self.layernorm_out = x.clone() # out
        x = self.ffn(x)
        ...
        # Obtain layernorm input and output data.
        layernorm_in_np, layernorm_out_np = self.get_layernorm_data()
        # Save gelu input and output data.
        self.save_data(self.layer_index, layernorm_in_np, layernorm_out_np, image_names, "./out_layernorm_2")

        return x, weights
    
    def get_layernorm_data(self):
        # Copying a tensor from GPU memory to CPU memory and converting it to a NumPy array.
        layernorm_in_cpu = self.layernorm_in.cpu()
        layernorm_out_cpu = self.layernorm_out.cpu() 
        layernorm_in_np = layernorm_in_cpu.numpy()
        layernorm_out_np = layernorm_out_cpu.numpy()

        return layernorm_in_np, layernorm_out_np
    
    def save_data(self, layer_index, layernorm_in_np, layernorm_out_np, image_names, output_dir):
        # If output_dir does not exist, create it
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Loop through batch_size and num_heads
        batch_size, width_len, height_len = layernorm_in_np.shape
        for i in range(batch_size):
            # Generate unique filenames for each sub-tensor
            image_name = image_names[i] if image_names else "unnamed"
            input_filename = f"{image_name}_layernorm_2_input_layer{layer_index}_batch_{i}.csv"
            output_filename = f"{image_name}_layernorm_2_output_layer{layer_index}_batch_{i}.csv"
            ...
            input_df = pd.DataFrame(layernorm_in_np[i])
            input_df.to_csv(input_filepath, index=False)

            output_df = pd.DataFrame(layernorm_out_np[i])
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, image_names=None):
        ...
        print('hidden_states_input:', hidden_states)
        encoded = self.encoder_norm(hidden_states)
        print('encoded_output:', encoded)
